...
- People Polkadot & Asset Hub Polkadot: deploy the Individuality scarcity stack, which turns credits earned by playing the game into collectible NFTs.
  - People Polkadot: `pallet-nft-credits` records the credits `pallet-game` awards, commits each block's awards to a Merkle root, and delivers the roots to Asset Hub over XCM. It serves the `NftCreditsApi` runtime API a wallet reads mint-ready inclusion proofs from. The credit bounds (`MaxCreditsPerBlock`, `MaxCreditBlocksPerClaimant`, `MaxRetainedAwardBlocks`) move here from `pallet-game`'s configuration, which now names the credit backend through `NftClaimCredits`.
  - Asset Hub Polkadot: `pallet-scarcity` holds the collectibles — one item per account, feeless rest-prioritised transfers through the new `AsScarcity` origin modifier, and aggregate per-collection storage deposits. `pallet-nft-claims` receives the credit trees, verifies an inclusion proof against the stored root and mints the selected item, resolving the claimant to either the signing account or the person its alias is bound to.
  - Asset Hub Polkadot: the `ScarcityCollection`, `ScarcityFactory` and `NftClaimsMinter` precompiles expose collections to `pallet-revive` contracts as ERC-721s and let a collection owner register it for claims, including a minter contract that picks the item.
  - `AsScarcity` joins transaction extension version 1 ahead of the nonce and payment checks. Version 0 is unchanged, so `transaction_version` does not move.
  - The scarcity stack's operational values are governance-adjustable through `pallet-parameters` (Fellowship or root, and Asset Hub's `technical_maintenance` track): the Scarcity storage-deposit rates, the purse failure-lock period and transfer-priority ceiling, and the nft-claims minter-contract weight and deposit ceilings on Asset Hub; the per-tree remote weight surcharge on People. The credit-tree and metadata bounds stay constants — they size stored encodings, which cannot change without a migration.
  - Purse transactions must be signed with a mortality era shorter than Scarcity's `LockPeriod` (60s), i.e. a period of at most 16 blocks on Asset Hub Polkadot's two-second blocks. The common 64-block wallet default is unsafe: an unexecuted transaction stays replayable until its era expires.
  - Note: `pallet-scarcity` has not been formally audited (see `AUDIT_STATUS.md` in `paritytech/individuality`). Its weights are still Substrate's kitchensink measurements, and the `indiv_pallet_game` and `indiv_pallet_nft_credits` weights are carried over from the Paseo reference runtimes; all three must be regenerated before release.
